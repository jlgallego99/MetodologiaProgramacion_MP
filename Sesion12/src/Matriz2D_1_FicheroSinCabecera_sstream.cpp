...
Matriz2D_1 :: Matriz2D_1(const char * nombre){
   ...
   TipoBase n;       // Valor de cada casilla
   string linea;     // Cada línea del fichero

   bool primera_linea = true;


   // Se procesa la primera línea únicamente para ver el número de columnas
   getline(fi, linea);
   filas = 1;
	while (!fi.eof() && primera_linea){
      ...
      while (iss >> n){ // Extraer un valor
         ...
         matriz[0][columnas - 1] = n;

      }

      getline(fi, linea);  // Nueva línea, hasta encontrar una no vacía

   }

   // Se procesa el resto del fichero
   while (!fi.eof()){
      ...
      if (iss >> n){
         ...
         filas++;          // Nueva fila
         Redimensiona();   // Memoria para la nueva fila

         // Se copian los valores de la linea en la matriz
         for (int i = 0 ; i < columnas ; i++){

            iss >> matriz[filas - 1][i];

         }

      }

      getline(fi, linea);  // Nueva línea
...
void Matriz2D_1 :: LeerMatriz2D_1(const char * nombre){
	...
	Matriz2D_1 m (nombre);  // Crea una matriz a partir del fichero

   (*this) = m;   // Copia la matriz en la implícita
...
void Matriz2D_1 :: Redimensiona(){

   // Nueva matriz copia de la original, con el nuevo tamaño
   Matriz2D_1 m_copia (filas, columnas);

   for (int i = 0 ; i < filas ; i++){

		memcpy(m_copia.matriz[i], matriz[i], columnas * sizeof(TipoBase));
