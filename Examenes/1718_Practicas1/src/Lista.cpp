...
void OrdenaIntercambioLista (Lista &l)
{
	...
	// Se comprueba previamente que la lista no esté vacía
	if (!ListaVacia(l)){
		
		int num_nodos = CuentaElementos(l);	// Número de nodos de la lista
		
		PNodo mayor;	// Controla el nodo mayor valor en cada pasada
		PNodo siguiente;

		for (int i = 0 ; i < num_nodos ; i++){	// Realiza num_nodos pasadas
			
			PNodo aux = l;	// Auxiliar al principio para recorrer los nodos 

			for (int j = 0 ; j < (num_nodos - i) ; j++){
				
				siguiente = aux->sig;	// Siguiente nodo al actual	 			

				if (aux->valor > siguiente->valor){
					
					TipoBase tmp = siguiente->valor;
					siguiente->valor = aux->valor;
					aux->valor = tmp;
			...
			}	// Se contemplan únicamente los elementos desordenados
		}
	}
} 
